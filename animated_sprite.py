...
class AnimatedSprite:
  ...
  def __init__(self, game, x, y, path='resources/sprites/player', ratio=cfg.PLAYER_SCALE, anim_speed = cfg.ANIM_SPEED):
    ...
    self.animation_time_last = self.game.time
  ...
  def get_anims(self, path):
    ...
    im = pygame.image.load(path+'/0.png')
    w, h = im.get_size()
    factor = cfg.WIDTH * self.ratio / w
    for file in sorted(files):
      if os.path.isdir(path+'/'+file):
        d[file] = self.get_anim_from_dir(path+'/'+file, factor)
    return d

  def get_anim_from_dir(self, path, factor=1):
    anim = []
    files = os.listdir(path)
    for idx, file in enumerate(sorted(files)):
      if os.path.isfile(path+'/'+file):
        i = pygame.image.load(path+'/'+file).convert_alpha()
        i = pygame.transform.scale_by(i, factor)
        anim.append(i)
    return anim
    ...
    for sprite in self.content:
      ...
      if type(sprite).__name__ in ['Weapon', 'Bullet', 'ExplodeBullet']:
        ...
        if type(sprite).__name__ == 'Weapon':
          if sprite.reloading:
            time = (self.game.time - sprite.reload_start) / sprite.reload
        ...
        if sprite.price:
          ...
          for point in ol:
            ...
            n += 1
          pygame.draw.polygon(screen, 'gold', ol, 2)

          price_im = font.render(str(sprite.price), True, 'gold')
          price_rect = price_im.get_rect(centerx=sprite.rect.centerx)
          price_rect.bottom = sprite.rect.top
          screen.blit(price_im, price_rect.move(cx, cy))
        screen.blit(sprite.image, sprite.rect.move(cx, cy))
      elif type(sprite).__name__ == 'Potion':
        ol = sprite.outline.copy()
        n = 0
        for point in ol:
          ol[n] = (point[0] + sprite.rect.x + cx, point[1] + sprite.rect.y + cy)
        ...
        if self.game.time - sprite.last_damage <= cfg.IMMORTALITY_TIME:
          im.set_alpha(150)
        if not sprite.orientation:
      ...
      #   pygame.draw.rect(screen, (255, 0, 0, 155), sprite.rect.move(cx, cy))
        ...
        if not sprite.orientation:
          im = pygame.transform.flip(im, True, False)
        tmp_rect = sprite.image.get_rect(bottomleft=sprite.rect.bottomleft)
        screen.blit(im, tmp_rect.move(cx, cy))
        pygame.draw.rect(screen, 'red', tmp)
      elif type(sprite).__name__ == 'Door':
        tmp = sprite.image.get_rect(bottomleft = sprite.rect.bottomleft)
        screen.blit(sprite.image, tmp.move(cx, cy))
      else:
        screen.blit(sprite.image, sprite.rect.move(cx, cy))

      if sprite == self.game.p.reachable_weapon and type(self.game.p.reachable_weapon).__name__ != 'Portal':
        ol = sprite.outline.copy()
        n = 0
        for point in ol:
          ol[n] = (point[0] + sprite.rect.x + cx, point[1] + sprite.rect.y + cy)
