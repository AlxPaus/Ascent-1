...
import queue
import copy
from room import portal, pattern1, pattern2, pattern3, merchant, treasure
...
def create_level(depth=4, levels_completed=0):
  g = rec_build_graph(depth)
  doors = [[], [], [], [], [], [], [], [], []]
  dists = bfs(g)
  # content
  content = [None for i in range(9)]
  ma, smax, med = dists[0], -1, dists[0]
  ma = max(dists)
  smax = ma-1
  med = ma//2  
  ...
  content[dists.index(ma)] = portal
  content[dists.index(smax)] = treasure
  content[dists.index(med)] = merchant
  for i in range(9):
    if content[i] is None and i != 1:
      content[i] = random.choice([pattern1,pattern2,pattern3])
  print(content)
