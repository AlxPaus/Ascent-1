...
from animated_sprite import Group, AnimatedSprite
...
class UI(Group):
  def __init__(self, game):
    ...
    self.content += [self.ability_bar]
    self.money = PlayerMoney(game, cfg.MEASURE*3.6, cfg.MEASURE*0.2)
    self.content += [self.money]
...
class WeaponBar:
  ...
  def update(self):
    if self.player.weapon_dict[self.num] != None:
      self.weapon_image = pygame.image.load(self.player.weapon_dict[self.num] + '/0.png')
      self.weapon_image = pygame.transform.scale_by(self.weapon_image, cfg.MEASURE / self.weapon_image.get_size()[0])
...
class PlayerMoney(AnimatedSprite):
  def __init__(self,game,x,y):
    super().__init__(game,x,y,path = 'resources/money_interface',ratio = cfg.PLAYER_SCALE*0.5)
    
    self.value = self.game.p.money
  ...
  def draw(self, screen):
    ...
    a_im = font.render(str(self.value), True, 'gold')
    screen.blit(a_im,(self.rect.right,self.rect.y))
    
class AbilityBar:
  def __init__(self, game):
    self.game = game
    self.back = pygame.transform.scale(pygame.image.load('resources/ability_bar.png'), (cfg.MEASURE*3, cfg.MEASURE*0.4))
    self.front = pygame.Surface((cfg.MEASURE*2.06, cfg.MEASURE*0.35))
    self.front.fill((0, 255, 255))
    self.image = self.back.copy()
    self.image.blit(self.front, (cfg.MEASURE*0.78, cfg.MEASURE*0.05))
    self.rect = self.image.get_rect(bottomright = (int(cfg.MEASURE*9),int(cfg.MEASURE*0.789)))
    

  def update(self):
    percent = min(1,(self.game.time - self.game.p.last_shockwave) / cfg.SHOCKWAVE_CD)
    self.front = pygame.Surface((cfg.MEASURE*2.06* percent, cfg.MEASURE*0.35))
    self.front.fill((0, 255, 255))
    self.image = self.back.copy()
    self.image.blit(self.front, (cfg.MEASURE*0.78, cfg.MEASURE*0.05))
