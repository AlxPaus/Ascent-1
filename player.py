...
class Player(AnimatedSprite):
    ...
    def find_closest_weapon(self):
      ...
      lw = s_lw[0]
      if ((lw.rect.centerx-self.rect.centerx)*(lw.rect.centerx-self.rect.centerx) + (lw.rect.centery-self.rect.centery) * (lw.rect.centery-self.rect.centery)) ** 0.5 <= cfg.MEASURE*1.2:
        self.reachable_weapon = lw
      else:
        self.reachable_weapon = None

    def pick_up_weapon(self):
      print(self.reachable_weapon)
      if self.reachable_weapon:
        lw = self.reachable_weapon
        if type(lw).__name__ == 'Chest':
          lw.open()
        elif type(lw).__name__ == 'Portal':
          lw.teleport()
        elif type(lw).__name__ == 'Potion' and self.money >= lw.price:
          lw.take_potion()
          
        elif self.money >= lw.price:
